...
@Component({
  ...
  template: `
    ...
    >
      @for (slice of slices; track slice.index) {
        <g
          [class]="sliceClasses()"
          [style.animation-delay.ms]="animated() ? (5 - slice.index) * -200 : null"
        >
          <path
            [attr.d]="slice.path"
            [attr.fill]="whiteSliceIndex === slice.index ? 'transparent' : 'currentColor'"
            stroke="transparent"
            stroke-width="1.75"
  ...
  `,
  host: {
    class: 'inline-flex items-center justify-center leading-none text-accent',
  },
...
export class PizzaLogo {
  ...
  protected sliceClasses(): string {
    return [
      'origin-center [transform-box:view-box]',
      this.animated() ? 'animate-[pizza-slice-step_1.2s_steps(6,end)_infinite]' : '',
    ]
      .filter(Boolean)
      .join(' ');
  }
